...
The Thomas Jefferson National Accelerator Facility (\abbr{TJNAF}\label{abbr:tjnaf}), shown in Fig.~\ref{fig:jlab.aerial}, also known as Jefferson Laboratory (\abbr{JLab}\label{abbr:jlab}) is home to three experimental halls, \desg{A}, \desg{B}, \desg{C}, and the Continuous Electron Beam Accelerator Facility,\cite{cebaf}(\abbr{CEBAF}\label{abbr:cebaf}, Fig.~\ref{fig:jlab.cebaf}).
%
The analysis performed in this work utilizes data collected from hall \desg{B} in the \abbr{CEBAF} Large Acceptance Spectrometer \abbr{CLAS}, during a run period from March 2008 - June 2008 designated with the name g12.

\begin{figure}\begin{center}
\includegraphics[width=0.6\columnwidth]{\grpath/jlab/jlab_arial_view_II.pdf}
\caption[\abbr{JLab} Aerial View (photograph)]{\label{fig:jlab.aerial}\cite{cebaf_flckr}Aerial view of Jefferson Laboratory (\abbr{JLab}) facing east.}
